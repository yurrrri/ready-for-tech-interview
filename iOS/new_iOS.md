...
*   자동 참조 카운팅(ARC)은 어떻게 동작하나요?
    *   ARC가 자동으로 메모리를 관리해주는데, 개발자가 여전히 메모리 관리에 신경 써야 하는 이유는 무엇일까요?
      - ARC는 코드의 정적 분석에만 의존하기 때문에, 두 개 이상의 인스턴스가 서로를 강하게 가리키는 '순환 참조(Retain Cycle)' 상황이 발생하면 참조 카운트가 영원히 0으로 떨어지지 않는 문제가 발생합니다.
      -  이로 인해 앱이 켜져 있는 동안 메모리가 계속 낭비되는 **메모리 누수(Memory Leak)**가 발생하므로, 개발자가 참조 구조를 올바르게 설계해야 합니다.
    *   강한 참조(Strong Reference), 약한 참조(Weak Reference), 미소유 참조(Unowned Reference)는 각각 언제 사용해야 하나요? 예를 들어 설명해주세요.
      - 강한 참조는 한 객체가 다른 객체를 명확하게 소유하거나 생명주기를 별도로 가져갈 때 사용합니다.
      - 약한 참조는 서로 연관되어 있지만 한 객체가 다른 객체보다 먼저 사라질 수 있을때 (nil) 사용합니다.
      - 미소유 참조는 서로 연관되어 있을 때 한 객체가 다른 객체보다 먼저 사라지지 않음을 보장할 수 있을때 사용하게 됩니다.
    *   순환 참조(Retain Cycle)는 무엇이며, 어떻게 발생하고 해결할 수 있나요? (클로저에서의 순환 참조 포함)
        ...
        *   self가 nil이 될 수 있는 상황과 될 수 없는 상황을 어떻게 구분하나요?
          - 코드 블럭이 동기적으로 그 자리에서 실행되고 끝나는 것이 명확할 때 self가 nil이 될 수 없지만, @escaping 클로저 혹은 비동기 작업으로 인해 클로저가 끝나고 나서도 실행되는 작업이 있을때 self가 nil이 될 수 있습니다.
        *   guard let self = self 패턴의 장단점은 무엇인가요?
            - 장점: 클로저 내부 코드 블록 전체에서 self? 라고 매번 옵셔널 체이닝을 쓰지 않아도 되므로 코드가 깔끔해집니다. 또한, guard문을 통과한 이후에는 클로저 실행이 끝날 때까지 self가 임시로 강한 참조 상태가 되어 중간에 메모리에서 사라지는 것을 방지(지연 해제)해 줍니다.
            - 단점: 클로저 내부 동작이 매우 오래걸리는 작업이라면, guard let self = self로 인해 클로저가 강하게 붙잡고있다는 점 때문에 (strong) 뷰컨트롤러가 해제 되더라도 클로저가 끝날때까지 메모리에 남아있을 수 있다는 단점이 있습니다.
...
- iOS의 메모리 구조는 크게 코드, 데이터, 힙 스택 영역 4가지 영역으로 나뉩니다.
- 코드 영역에는 프로그램의 코드가 들어가며, 데이터 영역에는 전역 변수와 정적 변수가 저장됩니다.
- 힙 영역에는 클래스 인스턴스나 클로저같은 참조 타입이 동적으로 할당되고, 스택에는 함수 호출 시 생성되는 지역변수, 혹은 struct나 enum 같은 값 타입이 저장됩니다.
- iOS는 이 동적인 Heap 영역의 메모리를 관리하기 위해 ARC(Automatic Reference Counting) 방식을 사용합니다.
- ARC는 객체의 참조 횟수를 카운팅하여, 이 참조 횟수(RC)가 0이 되면 자동으로 메모리를 해제하는 방식입니다.

*   힙(Heap)과 스택(Stack)의 차이점은 무엇인가요?
  - 힙 영역에는 클래스 인스턴스나 클로저같은 참조 타입이 동적으로 할당되고, 스택에는 함수 호출 시 생성되는 지역변수, 혹은 struct나 enum 같은 값 타입이 저장됩니다.
   *   함수 호출 시 스택 메모리는 어떻게 사용되나요? 재귀 함수 호출 시 스택 메모리 사용은 어떻게 될까요?
    - 함수가 호출될 때마다 해당 함수의 매개변수, 지역 변수, 복귀 주소 등을 포함한 '스택 프레임'이 스택 영역에 Push되고, 함수가 종료되면 Pop되어 메모리가 해제됩니다.
    - 재귀 함수의 경우 반환이 이루어지기 전에 함수가 연속적으로 호출되므로, 종료 조건에 도달할 때까지 스택 프레임이 메모리에 계속 누적되어 쌓이게 됩니다
   ...
   - 값 타입은 값을 복사하는 Deep Copy, 참조 타입은 주소값을 복사하는 Shallow Copy를 사용합니다. 다만 값 타입 중 일부 컬렉션 타입에 대해서 데이터를 수정하기 전까지는 참조타입과 같이 메모리 주소를 참조하고 있다가, 값을 수정하기 시작할 때 값을 복사하는 Copy-on-Write를 통해 복사하도록 하여 최적화합니다.
    *   스택 오버플로우(Stack Overflow)는 어떤 경우에 발생할 수 있나요?
    - 스택 오버플로우는 프로그램에 할당된 제한된 스택 메모리 용량을 초과하여 데이터가 쌓일 때 발생합니다.
    - 가장 대표적인 원인은 재귀 함수에서 탈출 조건(Base Case)을 명시하지 않았거나 잘못 설정하여 함수가 무한정 호출될 때 발생합니다.
...
- 동시성: 동시에 실행되는 것처럼 보이게 구성하는 것, 싱글 코어에서도 동작하며 CPU가 여러 스레드를 아주 짧은 시간 단위로 번갈아가며 실행하는 시분할 방식
- 병렬성: 실제로 여러 CPU가 동시에 작업을 실행하는것이며, 반드시 멀티 코어 환경이 필요

*   iOS 기기는 여러 개의 CPU 코어를 가지고 있는데, 이것이 동시성/병렬성 구현과 어떤 관련이 있나요?

- 상황에 따라 동시성과 병렬성을 모두 활용함. 실행해야하는 스레드가 코어 개수보다 많다면 동시성을 통해 번갈아 처리하고, 여유 코어가 있을 경우 여러 스레드를 각 코어에 배정하여 병렬처리
...
1. **서버 API가 사용자 이름을 때로는 보내고 때로는 보내지 않을 때, 이를 어떻게 처리해야 하나요?**
   ...
   - 강제 언래핑(!)을 사용했을 때 앱이 크래시되는 시나리오와 이를 방지하는 방법은 무엇인가요?
      - 서버가 사용자 이름을 보내지 않는 상태에서 강제 언래핑을 시도하게 되면 앱이 크래시나게 됩니다.
      - 강제 언래핑 대신, 옵셔널 바인딩 (if let / guard let)으로 안전하게 값을 꺼내오도록 하여 이를 방지해야 합니다.
      - 혹은 서버에서 값을 얻어오지 못할 때의 기본 값이 있다면, nil 코어레싱 (??)을 통해 기본값을 지정할 수도 있습니다.
...
4. **Swift에서 클로저(Closure)란 무엇이며, 어떻게 사용하나요?**
   - 클로저의 캡처(Capture) 기능은 무엇인가요?
     - 클로저가 정의된 스코프(범위) 내에 있는 외부 변수나 상수를 내부적으로 참조하여, 해당 변수가 속한 원래 스코프가 종료되어 메모리에서 사라지더라도 클로저가 실행되는 동안 그 값을 안전하게 유지하고 수정할 수 있게 만드는 기능입니다.
   - @escaping 클로저와 non-escaping 클로저의 차이점은 무엇인가요?
      - escaping 클로저는 함수가 리턴되어 종료된 이후에도 메모리에 남아있다가 나중에 실행될 수 있는 클로저로, 주로 비동기 작업에서 많이 쓰이게 됩니다.
      - non-escaping 클로저는 함수의 실행 흐름 안에서만 사용되어 함수가 리턴되기 전에 실행이 완료됩니다.
   - 트레일링 클로저(Trailing Closure) 문법은 어떤 경우에 유용한가요?
      - 아규먼트 레이블 안에 코드블럭을 쓰지 않아도 되기 때문에 가독성이 좋아집니다.
...
6. **Swift의 기본 데이터 타입과 컬렉션(Collection) 타입에는 어떤 것들이 있나요?**
   - 값 타입(Value Type)과 참조 타입(Reference Type)의 차이점은 무엇인가요?
      - 값 타입과 참조 타입의 가장 큰 차이점은 메모리 저장 위치와 복사 방식에 있습니다.
      - Swift에서 구조체, 열거형 같은 값 타입은 주로 스택(Stack) 영역에 저장되며, 다른 변수에 할당될 때 **실제 값이 복사(Deep Copy)**되어 독립적인 인스턴스를 유지합니다. 따라서 한쪽을 수정해도 다른 쪽에 영향을 주지 않습니다.
      - 반면 클래스나 클로저 같은 참조 타입은 힙(Heap) 영역에 실제 데이터가 저장되고, 스택에는 그 메모리 주소만 저장됩니다. 변수에 할당될 때 이 **주소값이 복사(Shallow Copy)**되므로, 여러 객체가 동일한 인스턴스를 공유하게 되어 한 곳에서의 변경이 다른 곳에도 영향을 미친다는 특징이 있습니다.
   - 구조체(Struct)와 클래스(Class)의 사용 시기는 어떻게 구분하나요?
   - 열거형(Enum)의 원시값(Raw Value)과 연관값(Associated Value)은 무엇인가요?
      - 원시값은 열거형의 각 케이스에 기본값을 지정해두어 매칭하기 쉽게하기 위해 주로 사용합니다. (미리 기본값 고정)
      - 연관값은 열거형의 각 케이스마다 부가적인 데이터를 함께 저장하고 사용하기 위해 사용합니다. (enum 객체를 생성하는 시점에 값 생성)
...
13. **Swift의 고차 함수(Higher-Order Functions)에 대해 설명해주세요.**
    - 함수를 파라미터로 사용하거나, 함수의 반환 결과가 함수인 함수
...
16. **Swift의 에러 처리 방법에 대해 설명해주세요.**
    - `throws`, `try`, `catch` 키워드의 사용 방법은 무엇인가요?
    - 옵셔널을 사용한 에러 처리와 `do-catch`를 사용하는 에러 처리의 차이는 무엇인가요?
    - 에러를 전파하는 방법은 무엇인가요?

17. **메모리 관리에서 강한 참조(Strong Reference)와 약한 참조(Weak Reference)의 차이점은 무엇인가요?**
    - 강한 참조 (Strong Reference)는 Swift의 기본 참조 방식입니다. 인스턴스를 가리킬 때 참조 카운트(RC)를 1 증가시키며, 해당 인스턴스의 소유권을 갖습니다. 참조 카운트가 물리적으로 0이 되어야만 메모리에서 해제되므로, 나를 가리키는 강한 참조가 하나라도 남아있다면 인스턴스는 메모리에 계속 유지됩니다.
    - 약한 참조 (Weak Reference)는 인스턴스를 가리키지만 참조 카운트(RC)를 증가시키지 않습니다. 소유권을 갖지 않는 형태입니다. 참조하고 있던 실제 인스턴스가 메모리에서 해제되면, ARC에 의해 자동으로 nil로 초기화(Zeroing)됩니다. 런타임에 값이 nil로 바뀔 수 있어야 하므로, 약한 참조는 항상 옵셔널 변수(weak var)로만 선언해야 합니다.
    - 순환 참조(Retain Cycle)가 발생하는 경우와 해결 방법은 무엇인가요?
       - 서로를 강하게 붙잡고 있는 상태를 말하며, 모두 nil로 만들어도 서로 붙잡고 있기 때문에 참조 카운트가 0이 되지 않아, 메모리 누수가 발생하는 현상입니다.
       - 한쪽을 weak (약한 참조로) 바꾸어 참조 카운트가 올라가지 않도록 합니다.
    - 클로저에서 `[weak self]`와 `[unowned self]`의 차이는 무엇인가요?
       - 약한 참조는 클로저가 실행되는 도중 혹은 전에 self가 해제 되면 자동으로 nil을 할당합니다.
       - 비소유 참조는 옵셔널을 할당할 수는 있지만 자동으로 nil을 할당하지는 않습니다. 또한, 클로저가 실행되기 도중 혹은 전에 self가 절대 죽지 않는다는 확신이 있을 때 사용하지만 한번 더 고려할 사항이 생기는 번거로움이 있어 프로젝트에서는 주로 weak self를 사용하는 편입니다.

19. **iOS 앱에서 Multi-threading을 구현하는 방법은 무엇인가요?**
    - `DispatchQueue`와 `OperationQueue`의 차이점은 무엇인가요?
    - 동시성 프로그래밍에서 Race Condition을 방지하는 방법은 무엇인가요?
    - 메인 스레드에서 UI 업데이트를 해야 하는 이유는 무엇인가요?

20. **UIKit에서 TableView와 CollectionView의 차이점은 무엇인가요?**
...
24. **Xcode에서 Instruments를 사용하여 앱의 성능을 분석하는 방법은 무엇인가요?**
...
25. **Swift의 제네릭(Generic)에 대해 설명해주세요.**
    - 제네릭을 사용하는 이유는 무엇인가요?
    로직을 짤때 타입만 다르고 로직은 동일할 때, 코드를 반복할 필요가 없이 모든 타입을 커버하도록 하여(즉 구체적인 타입을 명시하지 않고, 호출 시점에 타입이 결정) 코드의 재사용성을 극대화하기 위해 사용합니다.
    - 제네릭 타입 파라미터와 제약 조건을 설정하는 방법은 무엇인가요?
   제네릭 타입 파라미터의 경우 함수의 이름 마지막에 <>를 쓰고, 안에 타입 파라미터를 작성하면 됩니다. 타입 파라미터 작성 시, 대문자로 시작해야합니다.
   :로 제약을 줄 수 있으며, 프로토콜 제약 혹은 클래스 타입 제약을 줄 수 있습니다.
    - 제네릭을 사용할 때의 장점과 주의할 점은 무엇인가요?
   장점은 코드의 재사용성을 높일 수 있다는 점이며(하나의 함수/타입으로 여러 타입을 대응할 수 있음), 단점은 제네릭이 남용될 경우에 컴파일 시간이 길어질 수 있습니다.

26. **Swift의 클로저와 함수의 차이점은 무엇인가요?**
    - 클로저가 일급 객체(First-Class Citizen)인 이유는 무엇인가요?
       - 변수에 할당할 수 있고, 파라미터로 전달할 수 있고, 반환할 수 있다는 점에서 타입으로 취급되기 때문입니다.
    - 함수형 프로그래밍 패러다임에서 클로저가 어떻게 활용되나요?
      - 주로 고차함수에서 활용되고, 고차함수란 함수를 인자로 받거나 반환하는 함수를 의미합니다.

27. **동시성 프로그래밍에서 동기(Synchronous)와 비동기(Asynchronous)의 차이점은 무엇인가요?**
    - iOS에서 비동기 작업을 처리하는 방법은 무엇인가요?
    - 세마포어(Semaphore)와 뮤텍스(Mutex)의 차이점은 무엇인가요?

28. **GCD(Grand Central Dispatch)의 주요 개념과 사용 방법을 설명해주세요.**
    - 직렬(Serial) 큐와 동시(Concurrent) 큐의 차이는 무엇인가요?
    - 글로벌 큐(Global Queue)와 메인 큐(Main Queue)는 어떻게 다르나요?
    - DispatchWorkItem을 사용하는 방법은 무엇인가요?

29. **SwiftUI에서 @State 변수를 변경했는데 화면이 업데이트되지 않는다면 어떤 문제를 의심해야 하나요?**
...
30. **여러 화면에서 동일한 알림 기능을 사용해야 할 때, 싱글톤 패턴 대신 어떤 디자인 패턴을 고려해볼 수 있나요?**
...
- 강제 언래핑(Force Unwrapping)을 사용하면 안 되는 이유는 무엇인가요?
   - 해당 값이 옵셔널인데 강제 언래핑을 통해 꺼내게 되면 nil이 들어있는 상태라면 앱이 크래시나기 때문에 사용하면 안됩니다.
- 옵셔널 바인딩(Optional Binding)과 옵셔널 체이닝(Optional Chaining)의 차이점을 설명해주세요.
   - 옵셔널 바인딩은 Optional에 값이 있는지 확인하고, 있다면 꺼내오는 방식이며 옵셔널 체이닝은 체인 중간에 nil이 있으면 이후의 평가를 멈추고 nil을 반환하게 됩니다.
...
- 로컬 푸시 알림과 원격 푸시 알림(Remote Push Notification)의 차이점은 무엇인가요?
  - 로컬 푸시 알림은 외부 서버가 필요 없고, 앱 내부에서 특정 시간이나 주기, 위치 등을 기반으로 하여 알림을 발송하는 방식입니다.
  - 원격 푸시 알림은 자체 푸시알림을 발송하는 백엔드 서버와 APNs Key가 필요하며, 서버에서 푸시 알림을 보내는 방식입니다.
- 푸시 알림의 콘텐츠(Content)와 트리거(Trigger)는 어떤 역할을 하나요?
  - 콘텐츠는 알림창에 표시될 실제 내용을 담으며, 푸시 알림을 수신했을 때의 제목과 본문, 기타 개발 시 필요한 메타 데이터를 받게됩니다.
  - 트리거는 알림이 발송되는 조건을 의미합니다. 날짜라든지 지역, 시간 등을 트리거로 받을 수 있습니다.
- 사용자가 푸시 알림을 탭했을 때 앱의 동작을 처리하는 방법을 설명해주세요.
   - UNUserNotificationCenterDelegate 프로토콜을 구현해야 하는데, message의 내용을 받아 적절한 동작을 처리합니다.
