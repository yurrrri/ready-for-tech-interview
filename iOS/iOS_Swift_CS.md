...
- GCD에서는 async 코드를 통해 비동기 작업을 수행하며, async/await은 비동기 흐름을 마치 동기처럼 읽기 쉬운 코드로 표현할 수 있습니다.
...
- iOS에서는 각 앱을 별도의 프로세스로 실행하며, GCD를 통해 스레드 관리
...
- Main 스레드에서 시간이 오래 걸리는 작업을 진행하게 되면, UI 관련한 작업은 메인 스레드에서만 처리할 수 있으므로 UI 응답성이 저하되어 사용자 경험에 안좋은 영향을 미칠 수 있습니다.
...
#### GCD의 DispatchQueue 종류(Serial, Concurrent)와 사용 목적과 차이에 대해 설명해주세요.
