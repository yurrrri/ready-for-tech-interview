...
- 기본적으로 순수 함수를 활용하는 패러다임이고, 순수 함수를 기반으로 하기에 상태 변화를 최소화 하고, 데이터를 불변하게 다루며, 함수를 일급 객체로 취급하는 패러다임이다.
- 코드의 예측 가능성과 안정성을 높여준다.
- 함수를 호출하면 항상 동일한 결과를 반환하기 때문에, 함수를 디버깅하거나 테스트가 쉬워진다. 

#### 순수함수란?
- 동일한 입력에 대해 항상 동일한 결과를 반환하며, 사이드 이펙트가 없는 함수입니다.
