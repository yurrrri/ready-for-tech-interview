...
3. 구조체는 멤버와이즈이니셜라이저를 자동으로 제공하며, 클래스는 이와 달리 편의생성자와 필수생성자가 존재한다.
4. 구조체는 소멸자가 없으며 클래스는 소멸자가 존재한다.
5. 구조체는 원칙적으로 자기 자신의 속성값을 변경이 불가하고, 클래스는 변경이 가능하다. 만약 구조체에서 메소드가 속성을 변경할 경우 mutating 키워드를 붙여주어야한다.
...
- 다형성을 타입 변환과 관련지어 설명드리면,
클래스 인스턴스를 업캐스팅 혹은 다운캐스팅 함에 따라 여러가지 타입의 형태로 표현할 수 있고, 표현하는 타입에 따라 실행 가능한 메서드 종류나 메서드의 실행 형태가 다양할 수 있다는 속성입니다.
...
- 옵셔널 체이닝은 접근연산자 앞의 타입이 옵셔널 타입일 수 있음을 물음표(?)로 나타내는 문법적 표현이며, 닐 코얼레싱 오퍼레이터는 옵셔널의 값을 꺼내올 수 있으면 해당 값을 할당하고, 아니라면 지정한 기본값을 지정하겠다는 문법입니다.
- 옵셔널 체이닝은 반환값이 항상 옵셔널이기 때문에 결국에는 값을 접근하기 위해서는 옵셔널 바인딩 등의 방법을 통해 언래핑을 해야하며, 하나라도 nil이라면 그 뒤의 표현식을 거치지 않고 nil을 반환하므로 nil을 반환하는 경우에 대해 적절하게 대응해야합니다.
- 닐 코얼레싱 오퍼레이터는 기본값으로 지정할 값이 변수의 데이터 타입과 일치해야 한다는 점, 기본값을 어떻게 세팅할 것인지를 로직에 맞게 잘 설정해야합니다.
...
hashValue가 같아서 해시 충돌이 일어날 가능성이 있기 때문에, 추가로 동일한지 확인하는 Equatable 프로토콜을 채택하여 유일성을 보장하게 할 수 있습니다.

### 접근 제어자의 종류엔 어떤게 있는지 설명하시오.

접근 제어자에는 **open, public, internal, fileprivate, private** 가 있습니다.

open&public의 경우 **모듈 외부에서 접근이 가능하다는 공통점**이 있지만, **open의 경우 모듈 외부에서 해당 클래스를 상속하는 것이 가능하나** **public은 불가능하다는 차이점이 있습니다.**

internal은 따로 지정하지 않으면 디폴트로 설정되는 기본 접근 레벨로, 해당 모듈 전체에서 사용 가능합니다.

fileprivate은 지정된 요소가 구현된 소스파일 내부에서만 사용 가능합니다.

private는 기능을 정의하고 구현한 스코프(범위, 코드블럭) 내에서만 사용 가능합니다.
...
- Extension은 클래스, 구조체, 열거형 등의 타입에서, 기존 타입에 새로운 메소드를 추가하여 사용하는 '수평 확장' 방식입니다.

#### 상속과 Extension의 차이점은?

- 상속은 기존 타입의 속성과 메소드를 물려받아 새로운 타입이 추가되어, 해당 새로운 타입이 저장 속성을 추가하거나 메서드를 재정의, 혹은 메서드를 추가하는 방식입니다.
- 반면에 Extension은 타입을 추가하는 것이 아니라 기존 타입에 메서드를 추가하는 방식이기 때문에 메서드의 재정의나 저장 속성 추가가 불가능합니다.

#### Extension 내부에서 함수 override가 가능한가요?

- Extension 내부에서 함수 override는 불가능합니다.
- Extension 은 구조체나 클래스, 열거형의 기존 타입에서 기능을 확장하기 위한 것이지 변형하여 재정의하기 위한 목적이 아니기 때문에 불가능합니다.
...
- POP는 여러 프로토콜의 채택이 가능하며, 클래스 뿐만 아니라 값 타입의 모든 객체에서 프로토콜 채택이 가능합니다. 또한 상속이 아니기 때문에 필요한 속성이나 메소드만 채택하여 구현할 수 있다는 장점이 있습니다. 만약 POP에서 상속처럼 프로토콜을 채택하는 인스턴스에서 공통적으로 적용되는 기본값을 지정하고 싶다면, extension을 활용하여 protocol 요구사항의 기본값을 지정함으로써 반복되는 코드를 줄일 수 있습니다.

### Copy On Write는 어떤 방식으로 동작하는지 설명하시오.

https://babbab2.tistory.com/18

- Swift에서는 Array, Dictionary, Set과 같은 Collection Type을 복사할 때 일어나는 방식입니다.
- 본래 컬렉션 타입은 구조체로써 복사 시 값이 복사된 또 하나의 인스턴스가 생성되어야하지만, Copy On Write 방식을 사용하게 되면 해당 컬렉션 타입의 인스턴스 주소를 참조하고 있다가, 해당 인스턴스에 값의 변화가 생기면 그때 값을 복사하는 또 하나의 인스턴스가 생성되는 형태로 복사하게 됩니다.
- Copy On Write를 사용하게 되면, 불필요한 인스턴스의 복사를 줄임으로써 메모리를 효율적으로 관리할 수 있다는 장점이 있습니다.

### Swift에서는 복사 시 어떠한 원리로 복사되는지 설명해주세요

- 각 데이터 타입이 참조 타입인지, 값 타입인지에 따라 복사 방식이 다릅니다.
- struct나 열거형, 튜플 등과 같은 값 타입은 복사 시 값이 같은 새로운 인스턴스가 생성되어 복사되고,
- class나 클로저 같은 참조 타입은 인스턴스를 가리키는 주소값이 복사됩니다. 이 때 가리키는 주소값이 복사되므로 한번 더 가리킴으로써 reference count가 증가하게 되니 유의해야 합니다.

### Result타입에 대해 설명하시오.

- 에러를 던지는 함수에서 에러를 외부로 던지고 do-catch문으로 처리하는 것이 아니라
...
### available 키워드에 대해 설명하시오.

- @available의 경우 타입, 속성, 메서드 앞에서 available 키워드를 붙이고 버전을 명시하면, 해당 버전에서 코드의 사용 가능성을 결정하게 됩니다. 해당 키워드는 컴파일 타임에 사용 가능 여부를 확인할 수 있습닏나.
- #availbale의 경우 조건문과 같이 사용하며, 런타임 시 사용 가능성을 결정하게됩니다.

### Swift에서 Async/Await 기능이 도입되기 전에, 비동기(Asynchronous) 작업을 처리하는 방법에는 어떤 것들이 있나요?

- 기존에는 completionHandler, 즉 클로저를 콜백 함수로 해서 해당 클로저에 비동기 작업이 끝난 이후의 결과를 보내줬는데, 이러한 클로저가 지속적으로 반복되다보면 코드의 들여쓰기가 많아면서 가독성을 해쳤음
- 이 불편함을 개선해서 클로저로 전달하는 것이 아니라 return문을 통해 작업의 결과를 비동기적으로 전달하도록 개선된 문법이 async/await 기능

#### async, await이 무엇인지 알고있는지? 사용해본 경험이 있는지

- 비동기 작업을 진행하는 함수에 async 키워드를 붙이고, 해당 함수를 호출할 때 await을 붙여서 기존에 클로저를 활용하여 비동기작업의 결과를 전달받는 것이 아닌 return문을 통해 전달받을 수 있도록 하는 문법을 async, await이라고 함
...
- 함수의 input으로 사용되는 클로저 중, 해당 함수가 종료되어도 클로저를 실행할 수 있도록 하는 키워드인 escaping 키워드가 붙은 클로저를 탈출 클로저라고 합니다.
- 보통 함수의 내부 클로저를 외부 변수에 저장하거나, 비동기 프로그래밍할 때 유용하게 사용됩니다.

### KVO 동작 방식에 대해 설명하시오.

- KVO는 Key-Value-Observing 방식의 줄임말로써, 특정 키에 해당하는 value 값을 관찰하고 있다가 이 value에 변화가 감지가 되면 특정 코드가 실행되는 방식으로 동작하는 것을 의미합니다.
- 보통 앱의 각 요소끼리 (모델과 뷰와 같은) 소통하기에 적합하며, NSObject를 상속한 클래스에서만 KVO를 사용할 수 있고, 값의 변화를 감지하기 위해 observe 메소드를 사용하고 값의 변화가 발생하면 observe 메소드 코드블록 안의 코드가 실행됩니다.
- 또한 NSObject는 Objective-C 방식으로 동작하기 때문에 속성과 메소드에 @objc 키워드를 붙여주어야 올바르게 동작하게 됩니다.
...
- defer는 한번 실행이 되면 나중에 실행한다고 예약을 하는 것이기 때문에, defer문이 호출되기 전에 함수가 break나 return문을 통해 종료될 경우에는 호출되지 않습니다.
...
- Swift에서 클래스와 클로저같은 참조타입의 메모리 관리 측면에서 reference count의 증가/감소를 기반으로, 해제 시점을 자동으로 관리해주는 시스템 (reference count가 0이되면 인스턴스가 메모리에서 자동으로 해제되도록 도와주는 시스템)
...
- 이스케이핑 클로저가 클래스의 멤버로서 존재하고, 해당 클로저에서 인스턴스 자기 자신(self)를 가리키게 되면 강한 참조 사이클이 발생하게 됩니다.
